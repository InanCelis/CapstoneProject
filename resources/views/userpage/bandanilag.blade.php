       ...
       <div class="laman">
                ...
                @if($event_user_status-> count() > 0)
                  <div class="card bg-white shadow">
                    <div class="card-body">
                       ...
                       <a href="{{ route('bandanilag.pdf') }}" target="_blank" class="btn btn-sm  btn-dark" ><i class="far fa-file-pdf text-danger"></i> PDF Application Form</a><br> <small>Congratulations! Please click the button to download the form and bring it on the audition time. This will disappear when the admin removes it.</small> 
                    </div>
                  </div>
                @endif
            ...
            <div class="row">
                <div class="col-xl-12 order-xl-1">
                  <div class="card bg-white shadow">
                      <div class="card-body">
                        <form id="bandanilag" method="POST" enctype="multipart/form-data">
                            @csrf
                            <input type="hidden" name="form_name" id="form_name" value="{{ __('#bandanilag') }}">
                            <input type="hidden" name="route_name" id="route_name" value=" {{ route('bandanilag.store')}}">
                            <div class="row">
                             <div class="col-lg-12">
                              <center>
                                  <label class="heading-large text-center  mb-4">ENTRY APPLICATION FORM</label>
                                  <h2 class="heading-large text-center  mb-4">BANDANILAG (OPM BATTLE OF THE BANDS)</h2>
                              </center>
                             </div>
                            </div>
                      ...
                      @if($registeruser -> count() > 0)
                       @foreach($registeruser as $registered)
                            @foreach($user as $user_info)
                              ...
                              <br>
                              <div class="row">
                                  <button type="submit" class="btn btn-icon btn-3 btn-danger btn-submit" type="button">
...
if(count<=5)
{
      ...
      +      '<h4>ID Picture</h4>'
      +   '<div class="form-group">'
      +       '<input class="form-control form-control-alternative" accept="image/*" placeholder="Name of Person" type="file" name="member_image[]" id="member_image[]" required>'
...
@include('userpage.includes.bandanilag_member_cruds')
