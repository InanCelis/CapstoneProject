      ...
      <div class="laman">
       ...
       <div id="notice">
         ...
         @foreach($user as $user_info)
             @if (\Carbon\Carbon::parse($user_info->birthdate)->age <= 12)
              @if($event_user_status-> count() > 0)
                <div class="card bg-white shadow">
                   <div class="card-body">
                     <h3 class="text-danger"> <i class="fas fa-exclamation-triangle"></i> Important notice:</h3>
                     <a href="{{ route('singidolkid.pdf') }}" target="_blank" class="btn btn-sm  btn-dark" ><i class="far fa-file-pdf text-danger"></i> PDF Application Form</a><br> <small>Congratulations! Please click the button to download your form and bring it on the audition time. This will disappear when the admin removes it.</small> 
                   </div>
                </div>
              @endif
              @else
              @if($event_user_status-> count() > 0)
               <div class="card bg-white shadow">
                <div class="card-body">
                   <h3 class="text-danger"> <i class="fas fa-exclamation-triangle"></i> Important notice:</h3>
                   <a href="{{ route('singidol.pdf') }}" target="_blank" class="btn btn-sm  btn-dark" ><i class="far fa-file-pdf text-danger"></i> PDF Application Form</a><br> <small>Congratulations! Please click the button to download your form and bring it on the audition time. This will disappear when the admin removes it.</small> 
                </div>
               </div>
              @endif
             @endif
            ...
            <div class="col-xl-12 order-xl-1">
              <div class="card bg-white shadow">
                  <div class="card-body">
                    @foreach($user as $user_info)
                    @if (\Carbon\Carbon::parse($user_info->birthdate)->age <= 12)
                    <form id="singing_idol_form" method="POST" enctype="multipart/form-data">
                        @csrf
                        <input type="hidden" name="form_name" id="form_name" value=" {{ __('#singing_idol_form') }}">
                        <input type="hidden" name="form_name" id="route_name" value=" {{ route('anilag-singing-idol.store')}}">
                        <div class="row">
                         <div class="col-lg-12">
                          <center>
                              <label class="heading-large text-center  mb-4">ENTRY APPLICATION FORM</label>
                              <h2 class="heading-large text-center  mb-4">ANILAG SINGING IDOL (KIDS)</h2>
                          </center>
                         </div>
                        </div>
                        <div class="pl-lg-4">
                    ...
                    <form id="singing_idol_form" method="POST" enctype="multipart/form-data">
                        ...
                        <div class="row">
                         <div class="col-lg-12">
                          <center>
                              <label class="heading-large text-center  mb-4">ENTRY APPLICATION FORM</label>
                              <h2 class="heading-large text-center  mb-4">ANILAG SINGING IDOL (OPEN CATEGORY)</h2>
                          </center>
                         </div>
                        </div>
                        <div class="pl-lg-4">
                          ...
                          <div class="row">
                              ...
                              <button type="sumbit" class="btn btn-icon btn-3 btn-danger right btn-submit" type="button">
                                <span class="btn-inner--icon"><i class="ni ni-send" id="subIcon"></i></span>
                                <span class="btn-inner--text">Sumbit Registration</span>
                              </button>
