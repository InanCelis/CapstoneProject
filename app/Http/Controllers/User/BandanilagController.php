...
class BandanilagController extends Controller
{
    ...
    public function index()
    {
        ...
        $membercount = 1;
        $event_status = Tbl_event::where('id', 5)->value('event_status');
        
        return view('userpage.bandanilag', compact('user', 'registeruser', 'membercount', 'event_user_status','event_status','notifications'))->with('pagename', 'Bandanilag');
    }

 
    public function store(Request $request)
    {
       $eventStatus = Tbl_event::where('id', 5)->value('event_status');
        ...
        if($eventStatus == 'open')
        {
            ...
            if ($register_id === null) 
            {
                if (count($file['member_name']) < 3) {
                    return response()->json(['sorry' => 'Member should atleast 3 person!']);
                }
                ...
                if ($request->has('member_name')) 
                {
                    ...
                    $text = ['event' => auth()->user()->first_name.' '.auth()->user()->last_name.' registered in Bandanilag.'];
            ...
            else
            {
                ...
                $text = ['event' => auth()->user()->first_name.' '.auth()->user()->last_name.' updated group name in Bandanilag.'];
                event(new NotificationEvent($text));
    ...
    public function addnewmember(Request $request)
    {
        ...
        if($eventStatus == 'open')
        {
            ...
            if($countmember >=7)
            {
                return response()->json(['sorry' => 'Member maximum of 7 person only' ]);
            }
            else
            {
                $rules = array(
                ...
                $text = ['event' => auth()->user()->first_name.' '.auth()->user()->last_name.' add member in Bandanilag.'];
                event(new NotificationEvent($text));
    ...
    public function update(Request $request)
    {
        ...
        if($eventStatus == 'open')
        {
            ...
            $text = ['event' => auth()->user()->first_name.' '.auth()->user()->last_name.' update member data in Bandanilag.'];
            event(new NotificationEvent($text));
            ...
            if($eventStatus == 'open')
            {
               ...
               $register_id = Tbl_event_registration::where('user_id', auth()->id())
                ...
                if($countmember <=3)
                {
                    ...
                    $text = ['event' => auth()->user()->first_name.' '.auth()->user()->last_name.' deleted a member in Bandanilag.'];
                    event(new NotificationEvent($text));
